...
// add logic needed to retrieve the pizza-id data from the URL and make a GET request for the associated pizza
function getPizza() {
  // get id of pizza
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then(response => {
      // add error handling in case a user's pizza can't be found in the database
    ...
    .then(printPizza)
    // add .catch() so that any error takes the user back to the home page, using the window.history.back() method
    .catch(err => {
      console.log(err);
...
function handleNewCommentSubmit(event) {
  ...
  // add fetch() POST functionality to make sure users can create new comments
